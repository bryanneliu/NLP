...
def main():
    ...
    # S3 Outputs
    s3_asin_expansion_merged_synonym_csv = f"{s3_working_folder}dataset/synonym.csv/"
    s3_asin_expansion_click_strict_csv = f"{s3_working_folder}dataset/click.strict.csv/"
    s3_asin_expansion_click_loose_csv = f"{s3_working_folder}dataset/click.loose.csv/"
    s3_asin_expansion_merged_strict_csv = f"{s3_working_folder}dataset/strict.csv/"
    s3_asin_expansion_merged_loose_csv = f"{s3_working_folder}dataset/loose.csv/"

    output_csv(s3_asin_expansion_merged_synonym, s3_asin_expansion_merged_synonym_csv)
    output_csv(s3_asin_expansion_click_strict, s3_asin_expansion_click_strict_csv)
    output_csv(s3_asin_expansion_click_loose, s3_asin_expansion_click_loose_csv)
    output_csv(s3_asin_expansion_merged_strict, s3_asin_expansion_merged_strict_csv)
    output_csv(s3_asin_expansion_merged_loose, s3_asin_expansion_merged_loose_csv)
    ...
    logger.info("#################################################################")
    logger.info("Calculate output distribution for click strict expansion:")
    logger.info("#################################################################")
    calculate_output_distribution(s3_asin_expansion_click_strict, solr_index_asin_count)

    logger.info("#################################################################")
    logger.info("Calculate output distribution for click loose expansion:")
    logger.info("#################################################################")
    calculate_output_distribution(s3_asin_expansion_click_loose, solr_index_asin_count)

    logger.info("###################################################################")
    logger.info("Calculate output distribution for synonym & click strict expansions:")
    logger.info("###################################################################")
    calculate_output_distribution(s3_asin_expansion_merged_strict, solr_index_asin_count)

    logger.info("###########################################################################")
    logger.info("Calculate output distribution for synonym & click loose expansions:")
    logger.info("###########################################################################")
    calculate_output_distribution(s3_asin_expansion_merged_loose, solr_index_asin_count)
